...
class StaffWhInventoryCreate(BaseModel):
    sku: str | None = None
    variant_id: int | None = None
...
class StaffCatalogSkuOut(BaseModel):
    variant_id: int
    sku: str
    product: str
    color: str | None = None
...
class StaffCatalogSkuListResponse(BaseModel):
    items: list[StaffCatalogSkuOut]
    total: int
...
@router.get("/skus", response_model=StaffCatalogSkuListResponse)
def list_catalog_skus(
    ...
    exclude_in_stock: bool = Query(
        False,
        description="When true, hide SKUs already stocked in this warehouse",
    ),
    warehouse_id: int | None = None,
) -> StaffCatalogSkuListResponse:
    """Catalog SKUs for Add inventory dropdown (same source as admin catalog)."""
    ...
    stmt = (
        select(ProductVariant.id, ProductVariant.sku, product_label, ProductVariant.color, ProductVariant.size)
    ...
    if search and search.strip():
        like = f"%{search.strip()}%"
        stmt = stmt.where(or_(ProductVariant.sku.ilike(like), Product.name.ilike(like)))
    if exclude_in_stock and warehouse is not None:
        existing = select(WarehouseInventory.variant_id).where(
            WarehouseInventory.warehouse_id == warehouse.id
        )
        stmt = stmt.where(ProductVariant.id.not_in(existing))

    rows = db.execute(stmt.order_by(ProductVariant.sku.asc()).limit(500)).all()
    items = [
        StaffCatalogSkuOut(
            variant_id=int(vid),
            sku=sku or "",
            ...
            color=color,
            size=size,
            label=f"{sku} — {product}" if product else (sku or ""),
        )
        for vid, sku, product, color, size in rows
    ]
    return StaffCatalogSkuListResponse(items=items, total=len(items))
...
@router.post("", response_model=StaffWhInventoryOut, status_code=status.HTTP_201_CREATED)
def create_inventory(
    body: StaffWhInventoryCreate,
    db: Session = Depends(get_db),
    principal: TokenPrincipal = Depends(require_role("warehouse_manager")),
    _: User = Depends(get_current_warehouse_staff),
) -> StaffWhInventoryOut:
    ...
    elif body.sku and body.sku.strip():
        variant = db.scalar(
    ...
    if inv is not None:
        raise HTTPException(
            status_code=409, detail="Inventory already exists for this SKU — use Edit"
        )
    ...
    inv = WarehouseInventory(
        ...
        bin_location=(body.bin_location or "").strip() or None,
    )
    db.add(inv)
    try:
        db.commit()
...
def adjust_stock(
    ...
    if inv is None and body.sku:
        ...
        if inv is None and body.create_if_missing:
            inv = WarehouseInventory(
                warehouse_id=warehouse.id,
                variant_id=variant.id,
                on_hand=0,
    ...
    if body.on_hand is not None:
        ...
        body.bin_location is None
        and body.reorder_point is None
        and body.reserved is None
    ):
        raise HTTPException(
            status_code=422, detail="Provide on_hand, delta, bin_location, or reorder_point"
        )

    if body.reserved is not None:
        inv.reserved = body.reserved
    if body.reorder_point is not None:
        inv.reorder_point = body.reorder_point
    if body.bin_location is not None:
        inv.bin_location = body.bin_location.strip() or None
